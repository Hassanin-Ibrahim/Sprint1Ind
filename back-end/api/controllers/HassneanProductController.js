var mongoose = require('mongoose'),
    ...
    HassneanProduct = mongoose.model('HassneanProduct');
...
module.exports.getProduct = function(req, res, next) {
    ...
    HassneanProduct.findById(req.params.productId).exec(function(err, product) {
        if (err) {
            return next(err);
        }
        if (!product) {
            return res
                .status(404)
                .json({ err: null, msg: 'HassneanProduct not found.', data: null });
        }
        res.status(200).json({
            err: null,
            msg: 'HassneanProduct retrieved successfully.',
            data: product
        });
    });
};
...
module.exports.getProducts = function(req, res, next) {
    HassneanProduct.find({}).exec(function(err, products) {
        if (err) {
            return next(err);
        }
        res.status(200).json({
            err: null,
            msg: 'HassneanProducts retrieved successfully.',
            data: products
        });
    });
};
...
module.exports.createProduct = function(req, res, next) {
    ...
    HassneanProduct.create(req.body, function(err, product) {
        if (err) {
            return next(err);
        }
        res.status(201).json({
...
module.exports.updateProduct = function(req, res, next) {
    ...
    HassneanProduct.findByIdAndUpdate(
...
module.exports.deleteProduct = function(req, res, next) {
    ...
    HassneanProduct.findByIdAndRemove(req.params.productId).exec(function(
        err,
        deletedProduct
    ) {
        if (err) {
        ...
        res.status(200).json({
            err: null,
            msg: 'HassneanProduct was deleted successfully.',
            data: deletedProduct
        });
    });
};
